...
import { Button, useModal } from '@kiwanoproject/uikit'
...
const ContributeButton: React.FC<Props> = ({ poolId, ifo, publicIfoData, walletIfoData }) => {
  ...
  const handleContributeSuccess = async (amount: BigNumber, txHash: string) => {
    ...
    toastSuccess(
      ...
      <ToastDescriptionWithTx txHash={txHash}>
        {t('You have contributed %amount% WANO to this IFO!', {
  ...
  return (
    <Button
      ...
      disabled={isDisabled}
    >
      {isMaxCommitted && publicIfoData.status === 'live' ? t('Max. Committed') : t('Commit WANO')}
    </Button>
  )
}
